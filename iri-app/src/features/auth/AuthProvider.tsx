...
import { syncAnalyticsOptOut } from '@/features/analytics/track';
...
const PROFILE_COLUMNS =
  'id, display_name, kcal_goal, protein_goal_g, carbs_goal_g, fat_goal_g, water_goal_ml, water_glass_ml, streak_count, streak_longest, allergies, diet_preference, birth_year, start_weight_kg, target_weight_kg, onboarding_completed_at, timezone, push_broadcast, push_streak, push_meal_evening, push_water, push_weekly, reminder_evening_time, analytics_opt_out';
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p]) => {
        if (cancelled) return;
        setProfile(p);
        // Widerspruch gegen die Nutzungsstatistik haengt am Profil, damit er auf
        // jedem Geraet gilt. Der Zaehler liest ihn lokal — hier wird gespiegelt.
        if (p) void syncAnalyticsOptOut(p.analytics_opt_out);
      })
